...
export const settings = createModel<SettingsState>()({
    ...
    effects: (dispatch) => ({
        ...
        async playMusic() {
            const { settings: { setMusicState, } } = dispatch as Dispatch
        ...
        async toggleMusic(_, rootState) {
            const { settings: { stopMusic, playMusic, setMusicState } } = dispatch as Dispatch
            const { settings: { isMusicOn } } = rootState as RootState
